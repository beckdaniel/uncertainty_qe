...
def load_and_report(model_name, kernel, warp, ard, pretrain):
    """
    We load the pretrained models and predict using
    the median.
    """
    dataset_dir = os.path.join(MODEL_DIR, DATASET)
    pretrain_dir = os.path.join('..', 'models', pretrain, DATASET)
    try: 
        os.makedirs(dataset_dir)
    except OSError:
    ...
    for fold in xrange(10):
        ...
        params_file = os.path.join(pretrain_dir, str(fold), 'params')
        output_dir = os.path.join(dataset_dir, str(fold))
        gp = util.train_gp_model(train_data, kernel, warp, ard, params_file, preload=True)
        metrics = util.get_metrics(gp, test_data, median=True)
...
WARP = sys.argv[3]
ARD = sys.argv[4]
PRETRAIN_NAME = '_'.join([KERNEL, WARP, ARD])
MODEL_NAME = '_'.join([KERNEL, WARP, ARD, 'median'])
...
if KERNEL == 'split':
    ...
    load_and_report(MODEL_NAME, KERNEL, WARP, ARD, PRETRAIN_NAME)
